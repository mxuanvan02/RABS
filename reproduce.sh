...
echo "    - urgency ablation (baseline for the Table 4 cross-check)"
python code/run_rabs_era5_ablation.py
echo "    - scalability, 20 real stations (Table 3)"
python code/run_rabs_era5_scaling.py
echo "    - extreme-burst sensitivity: full baseline comparison"
python code/run_rabs_era5_main_extreme.py
echo "    - risk-channel decomposition, 2 regimes"
python code/risk_channel_decomposition.py
echo "    - risk-signal diagnostic: p vs delta spread/saturation"
python code/risk_signal_diagnostic.py
echo "    - surrogate-vs-realized calibration"
python code/surrogate_calibration.py
...
# Table 4 must be written AFTER make_era5_tables.py, which emits the
# legacy 5-row ablation table. make_ablation_table.py rebuilds it on the
# consistent deployed raw-p basis (8 rows) and aborts if any overlapping value
# drifts from the baseline ablation summary.
python code/make_ablation_table.py
...
echo "   outputs/rabs/rabs_era5_extreme_summary.csv    (extreme burst)"
echo "   outputs/rabs/rabs_risk_decomposition.csv        (risk decomposition)"
echo "   outputs/rabs/rabs_risk_signal_diagnostic.csv    (risk-signal diagnostic)"
echo "   outputs/rabs/rabs_ablation_matrix.csv           (Table 4 source)"
echo "   outputs/rabs/rabs_surrogate_calibration.csv     (bound calibration)"
