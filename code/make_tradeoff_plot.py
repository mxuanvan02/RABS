...
Output          : outputs/figures/tradeoff_plot.pdf (+ .png for inspection)
...
# Output to a repo-relative outputs/figures/ dir by default. Set RABS_FIG_DIR
...
ax.text(0.95, max(float(r["objective_mean"]) for r in sev.values()),
...
for key, label, *_ in SPEC:
    if key in sev:
        r = sev[key]
        print(f"  {label:22s} Bw={float(r['avg_bandwidth_mean']):5.2f}  Obj={float(r['objective_mean']):.4f}")
