#!/usr/bin/env python3
"""FINAL Table-4 generator: risk/urgency decomposition of the deployed RABS-PD.
...
Every ablation row removes exactly ONE component from that deployed system:
  ...
  deployed       : the submitted RABS-PD (reference row)
...
Runs severe_burst (main table) and extreme_burst (text/response letter).
Cross-checks the deployed row and the VoU/-deviation/-AoI severe rows against
the submitted rabs_era5_ablation_summary.csv so numbers cannot drift.
...
def main():
    ...
    with (OUT / 'rabs_q4_final_matrix.csv').open('w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)

    # --- cross-check vs submitted ablation (severe_burst) ---
    old = {(r['network'], r['variant']): r for r in csv.DictReader(
        (OUT / 'rabs_era5_ablation_summary.csv').open(newline='', encoding='utf-8'))}
    # submitted: full=vou, raw_p=deployed, no_risk=no_rank_risk(VoU==raw when w_risk=0),
    #            no_dev, no_aoi (VoU-based in submission)
    checks = [('vou', 'full'), ('deployed', 'raw_p'), ('no_rank_risk', 'no_risk')]
    print('\n--- cross-check severe_burst vs submitted (deployed-basis rows) ---')
    ok = True
    ...
    for new_v, old_v in checks:
        for m in ('objective', 'missed_pct', 'cvar95_loss'):
            ...
            same = abs(a - b) < 5e-4
            ok &= same
            print(f"  {new_v:<14} {m:<10} new={a:.4f} submitted={b:.4f} {'OK' if same else 'CHECK'}")
    ...
    L.append(r'\caption{Component ablation of the deployed RABS-PD (severe burst; the same ordering holds under extreme burst in the released ablation). Lower is better; \best{} marks the best objective. The risk channels are mutually redundant on this deterministic replay; the deviation and AoI terms are load-bearing.}')
