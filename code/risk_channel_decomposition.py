#!/usr/bin/env python3
"""Ablate all three risk channels of RABS-PD, on two channel regimes.
...
Regimes: severe_burst (default) + extreme_burst (harsher, ~48% overall loss).
...
Deterministic. Writes outputs/rabs/rabs_risk_decomposition.csv.
"""
...
def main():
    ...
    with (OUT / 'rabs_risk_decomposition.csv').open('w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)
    with (OUT / 'rabs_risk_decomposition_stats.csv').open('w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=list(stat_rows[0].keys())); w.writeheader(); w.writerows(stat_rows)
    print('WROTE', OUT / 'rabs_risk_decomposition.csv')
